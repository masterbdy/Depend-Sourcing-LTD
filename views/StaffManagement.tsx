...
const StaffManagementView: React.FC<StaffProps> = ({ staffList = [], setStaffList, role, expenses = [], advances = [], setAdvances, currentUser, onUpdatePoints, highlightStaffId, setHighlightStaffId }) => {
  ...
  return (
    <div className="space-y-6">
                ...
                {isExpanded ? (
                   <div className="relative bg-white dark:bg-gray-800 rounded-3xl shadow-2xl border border-gray-200 dark:border-gray-700 overflow-hidden h-full animate-in fade-in zoom-in duration-300 flex flex-col">
                      ...
                      <div className="p-6 flex-1 flex flex-col gap-6 overflow-y-auto custom-scrollbar">
                          ...
                          <div className="mt-2 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-4 cursor-default" onClick={e => e.stopPropagation()}>
                              <div className="flex items-center gap-2 mb-3">
                                  <StickyNote className="w-4 h-4 text-indigo-500" />
                                  <h4 className="text-xs font-black text-gray-700 dark:text-gray-300 uppercase tracking-widest">Notes</h4>
                              </div>
                              ...
                              <div className="bg-white dark:bg-gray-900 rounded-lg p-3 border border-gray-200 dark:border-gray-700 mb-4 shadow-sm">
                                  ...
                                  {newNoteImage && (
                                      <div className="relative inline-block mb-2">
                                          <img src={newNoteImage} alt="Attachment" className="h-16 rounded-md object-cover border border-gray-200 dark:border-gray-700" />
                                          <button onClick={() => setNewNoteImage(null)} className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full p-1 hover:bg-red-600 transition-colors">
                                              <X className="w-3 h-3" />
                                          </button>
                                      </div>
                                  )}
                                  <div className="flex items-center justify-between border-t border-gray-100 dark:border-gray-800 pt-2">
                                      ...
                                      <input type="file" hidden accept="image/*" ref={noteFileRef} onChange={handleNoteImageUpload} />
                                      <button 
                                          type="button"
                              ...
                              <div className="space-y-3 max-h-48 overflow-y-auto custom-scrollbar pr-1">
                                  {staff.notes?.map((note, index) => (
                                      <div key={`${note.id}-${index}`} className="bg-white dark:bg-gray-900 rounded-lg p-3 border border-gray-100 dark:border-gray-800 shadow-sm">
                                          ...
                                          <p className="text-xs text-gray-700 dark:text-gray-300 whitespace-pre-wrap leading-relaxed">{note.text}</p>
                                          {note.imageUrl && (
                                              <img 
                                                  src={note.imageUrl} 
                                                  alt="Attachment" 
                                                  ...
                                                  onClick={() => setViewingNoteImage(note.imageUrl as string)}
                                              />
                                          )}
                                      </div>
                                  ))}
                                  {(!staff.notes || staff.notes.length === 0) && (
                                      <div className="text-center py-4 text-gray-400 text-[10px] font-medium">No notes available.</div>
                                  )}
                              </div>
                          </div>
      ...
      {viewingNoteImage && createPortal(
          <div className="fixed inset-0 z-[300] flex items-center justify-center bg-black/90 p-4" onClick={() => setViewingNoteImage(null)}>
              <div className="relative max-w-4xl w-full h-full flex justify-center items-center">
                 <button 
                    ...
                    onClick={(e) => { e.stopPropagation(); setViewingNoteImage(null); }}
                 >
                    <X className="w-6 h-6" />
                 </button>
                 <img src={viewingNoteImage} alt="Full View" className="max-w-full max-h-full object-contain rounded-xl shadow-2xl" onClick={e => e.stopPropagation()} />
              </div>
          </div>,
          document.body
      )}
