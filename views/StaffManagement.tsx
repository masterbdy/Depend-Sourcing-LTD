...
const StaffManagementView: React.FC<StaffProps> = ({
  staffList = [],
  setStaffList,
  role,
  expenses = [],
  ...
  const [viewingNoteImage, setViewingNoteImage] = useState<string | null>(null);

  // Added search for history modal
  ...
  const [giftPointData, setGiftPointData] = useState<{
    staffId: string;
    points: number;
  }>({ staffId: "", points: 5 });
  ...
  const formatCurrency = (amount: number) => {
    ...
    return (
      <span
        className={`font-mono font-black tracking-tighter flex items-baseline truncate text-xl md:text-2xl ${colorClass}`}
        title={`৳${absAmount}`}
      >
  ...
  const formatCurrencyLight = (amount: number, colorClass: string) => {
    ...
    return (
      <span
        className={`font-mono font-black text-xl md:text-2xl tracking-tighter flex items-baseline truncate ${colorClass}`}
        title={`৳${absAmount}`}
      >
        {isNegative && <span className="mr-1">-</span>}
  ...
  const saveNote = (staffIdToUpdate: string) => {
    if (!newNoteText.trim() && !newNoteImage) return;

    setStaffList((prev) =>
      prev.map((s) => {
        if (s.id === staffIdToUpdate) {
          return {
            ...s,
            notes: [
              {
    ...
    if (editingStaff) {
      ...
      if (isDuplicate) {
        const confirmAdd = window.confirm(
          "এই নামের বা নাম্বারের একজন মেম্বার বা ডুপ্লিকেট ইতিমধ্যে সিস্টেমে থাকতে পারে! আপনি কি তারপরও নতুন যুক্ত করতে চান?",
        );
  ...
  const openEdit = (staff: Staff) => {
    ...
    setFormData({
      ...
      requiresCheckOutLocation:
        staff.requiresCheckOutLocation !== undefined
          ? staff.requiresCheckOutLocation
          : true,
  ...
  const confirmDelete = () => {
    if (deleteConfirmId && currentUser) {
      let isValid = false;
      ...
      if (adminStaff) {
        ...
        currentUser.toLowerCase() === "ispa" &&
        deletePassword === "ayaan"
      ) {
        isValid = true;
      } else if (
        currentUser.toLowerCase() === "mehedi" &&
        deletePassword === "mehedi@60"
      ) {
        isValid = true;
      }
  ...
  const confirmStatusChange = () => {
    if (statusConfirmData) {
      setStaffList((prev) =>
        prev.map((s) =>
          s && s.id === statusConfirmData.id
            ? {
                ...s,
                status: statusConfirmData.newStatus,
  ...
  const getCurrentLocation = (slot: 1 | 2) => {
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        alert(
          `Location ${slot} Set: ${position.coords.latitude}, ${position.coords.longitude}`,
        );
      },
      (error) => {
  ...
  const handleGivePoints = (e: React.FormEvent) => {
    e.preventDefault();
    if (!canManageMoney || !onUpdatePoints) return;
    const points = Number(giftPointData.points);
    if (points < 1) return alert("পয়েন্ট ১ এর বেশি হতে হবে।");
    if (pointMode === "PENALTY") {
      setIsGiftPointModalOpen(false);
      setTimeout(() => setShowPenaltyConfirm(true), 200);
      return;
    }
    onUpdatePoints(giftPointData.staffId, points, "ADMIN_REWARD");
    setIsGiftPointModalOpen(false);
  ...
  const executePenalty = () => {
    ...
    setTimeout(
      () => alert("পয়েন্ট পেনাল্টি কার্যকর হয়েছে (পয়েন্ট কাটা হয়েছে)।"),
      100,
    );
  };
  ...
  const handleGiveAdvance = (e: React.FormEvent) => {
    ...
    if (Number(advanceFormData.amount) <= 0)
      return alert("দয়া করে সঠিক টাকার পরিমাণ লিখুন (০ এর বেশি)।");
    const staff = (staffList || []).find(
      (s) => s.id === advanceFormData.staffId,
    );
    if (!staff) return;
  ...
  const handleRepay = (e: React.FormEvent) => {
    ...
    if (Number(repayFormData.amount) <= 0)
      return alert("টাকার পরিমাণ ০ এর বেশি হতে হবে।");
    ...
    const adjustmentType =
      repayFormData.type === "SALARY" ? "SALARY" : "REGULAR";
    const notePrefix =
      adjustmentType === "SALARY" ? "[SALARY ADJ]" : "[REGULAR ADJ]";
    ...
    alert(
      `${staff.name}-এর ${adjustmentType === "SALARY" ? "বেতন" : "রেগুলার হিসাব"} থেকে ৳${repayFormData.amount} সমন্বয় করা হয়েছে।`,
    );
  };
  ...
  const filteredStaff = useMemo(() => {
    let result = (staffList || []).filter((s) => {
      ...
      const matchesSearch =
        ...
        (s.email && s.email.includes(searchLower)) ||
  ...
  const stats = {
    ...
    active: (staffList || []).filter(
      (s) => s && !s.deletedAt && s.status === "ACTIVE",
    ).length,
    inactive: (staffList || []).filter(
      (s) => s && !s.deletedAt && s.status === "DEACTIVATED",
    ).length,
  };
  const getStaffFinancials = (staffId: string) => {
    const safeExpenses = expenses || [];
    const safeAdvances = advances || [];
    const staffExpenses = safeExpenses.filter(
      (e) => e && e.staffId === staffId && !e.isDeleted,
    );
    const approved = staffExpenses
      .filter((e) => e.status === "APPROVED")
      .reduce((sum, e) => sum + Number(e.amount || 0), 0);
    const staffAdvances = safeAdvances.filter(
      (a) => a && a.staffId === staffId && !a.isDeleted,
    );
    const totalRegularAdv = staffAdvances
      .filter((a) => a.type !== "SALARY")
      .reduce((sum, a) => sum + Number(a.amount || 0), 0);
    const totalSalaryAdv = staffAdvances
      .filter((a) => a.type === "SALARY")
      .reduce((sum, a) => sum + Number(a.amount || 0), 0);
  ...
  const handleRecoverMissingStaff = () => {
    ...
    (expenses || []).forEach((e) => {
      if (
        e.staffId &&
        !staffList.some((s) => s.id === e.staffId && !s.isHardDeleted)
      ) {
    ...
    (advances || []).forEach((a) => {
      if (
        a.staffId &&
        !staffList.some((s) => s.id === a.staffId && !s.isHardDeleted)
      ) {
    ...
    if (
      confirm(
        `${missingStaffIds.size} জন ইউজারের প্রোফাইল পাওয়া গেছে যা ডিলিট হয়ে গিয়েছিল। আপনি কি এগুলো রিকভার করতে চান?`,
      )
    ) {
  ...
  const handleMergeProfiles = (e: React.FormEvent) => {
    ...
    if (!sourceStaffId || !targetStaffId)
      return alert("দয়া করে উভয়েই প্রোফাইল নির্বাচন করুন।");
    if (sourceStaffId === targetStaffId)
      return alert("একই প্রোফাইল মার্জ করা যাবে না।");
    ...
    if (currentUser) {
      ...
      if (adminStaff) {
        const validPassword = adminStaff.password || `${adminStaff.name}@`;
        if (mergePassword === validPassword) isValid = true;
      } else if (
        currentUser.toLowerCase() === "ispa" &&
        mergePassword === "ayaan"
      ) {
    ...
    if (!sourceStaff || !targetStaff) return alert("স্টাফ পাওয়া যায়নি।");
    if (!setExpenses || !setMovements) return alert("ডাটা আপডেট ফাংশন নেই।");

    if (
      !confirm(
        `আপনি কি নিশ্চিত যে ${sourceStaff.name}-এর সকল লেনদেন এবং বিল ${targetStaff.name}-এর প্রোফাইলে যুক্ত করতে চান?\n\nমার্জ করার পরে ${sourceStaff.name}-এর প্রোফাইলটি ডিলিট হয়ে যাবে।`,
      )
    )
      return;
    ...
    setAdvances((prev) =>
      prev.map((a) =>
        a.staffId === sourceStaffId
          ? { ...a, staffId: targetStaffId, staffName: targetStaff.name }
          : a,
      ),
    );
    ...
    setExpenses((prev) =>
      prev.map((ex) =>
        ex.staffId === sourceStaffId
          ? { ...ex, staffId: targetStaffId, staffName: targetStaff.name }
          : ex,
      ),
    );
    ...
    setMovements((prev) =>
      prev.map((m) =>
        m.staffId === sourceStaffId
          ? { ...m, staffId: targetStaffId, staffName: targetStaff.name }
          : m,
      ),
    );

    // 4. Update StaffList (soft delete source)
    setStaffList((prev) =>
      prev.map((s) =>
        s.id === sourceStaffId
          ? { ...s, deletedAt: new Date().toISOString() }
    ...
    setMergePassword("");
    alert(
      `সফলভাবে ${sourceStaff.name}-কে ${targetStaff.name}-এর সাথে মার্জ করা হয়েছে!`,
    );
  };
    ...
    <div className="space-y-6">
        ...
        {role === UserRole.ADMIN && (
          <div className="flex flex-wrap items-center gap-2">
            <button
              ...
              title="দুটি প্রোফাইল একত্র করুন"
            >
              <RefreshCw className="w-4 h-4" />
              <span className="hidden md:inline text-sm">মার্জ প্রোফাইল</span>
            </button>
            <button
              onClick={handleRecoverMissingStaff}
              className="bg-amber-100 text-amber-700 px-4 py-2.5 rounded-xl font-bold shadow-sm hover:bg-amber-200 transition-all flex items-center gap-2"
              title="হারানো প্রোফাইল রিকভার করুন"
            >
            ...
            <button
              ...
              className="group relative overflow-hidden bg-gradient-to-r from-indigo-600 to-violet-600 text-white px-5 py-2.5 rounded-xl font-bold shadow-lg shadow-indigo-200 hover:shadow-indigo-300 transition-all active:scale-95 flex items-center gap-2"
            >
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
        {filteredStaff.map((staff, index) => {
          ...
          const { balance, totalSalaryAdv, totalRegularAdv, approved } =
            getStaffFinancials(staff.id);
          const safeName = staff.name || "Unknown";
          const safeId = staff.staffId || "N/A";
          const isActive = staff.status === "ACTIVE";
          const joinDate = new Date(staff.createdAt).toLocaleDateString(
            "bn-BD",
            ...
            >
              {isExpanded ? (
                <div className="relative bg-white dark:bg-gray-800 rounded-3xl shadow-2xl border border-gray-200 dark:border-gray-700 overflow-hidden h-full animate-in fade-in zoom-in duration-300 flex flex-col">
                  <div
                    className={`h-28 w-full relative shrink-0 ${isActive ? "bg-gradient-to-r from-violet-600 to-indigo-600" : "bg-gray-700"}`}
                  >
                    <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/carbon-fibre.png')] opacity-20"></div>
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        toggleExpand(staff.id);
                      }}
                    ...
                    <div className="absolute top-4 left-5 flex items-center gap-2">
                      <span
                        className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest bg-white/20 text-white backdrop-blur-md border border-white/30 shadow-sm flex items-center gap-1`}
                      >
                      ...
                      {staff.workLocation === "FIELD" && (
                        <span className="text-[10px] font-bold text-white bg-orange-500/80 px-2 py-0.5 rounded border border-white/20">
                          FIELD
                        </span>
                      )}
                    </div>
                  ...
                  <div className="px-6 flex flex-col md:flex-row gap-5 -mt-10 relative z-10">
                    <div className="relative shrink-0 group/avatar">
                      <div
                        className={`w-28 h-28 rounded-2xl border-[4px] border-white dark:border-gray-800 shadow-2xl overflow-hidden bg-white relative ${isActive ? "" : "grayscale"}`}
                      >
                        {staff.photo ? (
                          <img
                            src={staff.photo}
                      ...
                      <div
                        className={`absolute bottom-2 right-2 w-5 h-5 rounded-full border-2 border-white dark:border-gray-800 shadow-sm ${isActive ? "bg-green-500 animate-pulse" : "bg-red-500"}`}
                      ></div>
                    </div>
                    <div className="pt-12 md:pt-11 flex-1">
                      ...
                      <p className="text-sm font-bold text-indigo-600 dark:text-indigo-400 uppercase tracking-wide mb-3">
                        {staff.designation}
                      </p>
                      <div className="flex flex-wrap gap-2">
                        ...
                        <span className="bg-yellow-50 text-yellow-700 border border-yellow-200 px-2.5 py-1 rounded-lg text-xs font-bold flex items-center gap-1">
                          ...
                          {formatPoints(staff.points || 0)} pts
                        </span>
                      </div>
                    </div>
                  </div>
                  <div className="p-6 flex-1 flex flex-col gap-6 overflow-y-auto custom-scrollbar">
                    ...
                    >
                      ...
                      <div className="flex justify-between items-start mb-8 relative z-10">
                        <div className="flex items-center gap-2 opacity-80">
                          ...
                          <div>
                            <span className="text-[10px] font-black uppercase tracking-[0.2em] text-indigo-200 block">
                              Current Balance
                            </span>
                            <span className="text-[8px] font-bold text-gray-400">
                              Real-time Data • Click for History
                            </span>
                          </div>
                        </div>
                        <div className="text-right">
                          ...
                          <span
                            className={`px-2 py-0.5 rounded text-[10px] font-black border ${balance < 0 ? "bg-red-500/20 text-red-300 border-red-500/30" : "bg-emerald-500/20 text-emerald-300 border-emerald-500/30"}`}
                          >
                      ...
                      <div className="grid grid-cols-2 gap-4 relative z-10">
                        <div className="flex flex-col">
                          <p className="text-[9px] font-extrabold text-gray-500 uppercase tracking-[0.1em] mb-1">
                            Total Balance
                          </p>
                        ...
                        <div className="flex flex-col">
                          ...
                          {formatCurrencyLight(approved, "text-red-300")}
                        </div>
                        <div className="flex flex-col">
                          <p className="text-[9px] font-extrabold text-gray-500 uppercase tracking-[0.1em] mb-1">
                            Salary Adv
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="p-3 bg-gray-50 dark:bg-gray-700/50 rounded-xl border border-gray-100 dark:border-gray-600 flex items-center gap-3">
                        <div className="bg-white dark:bg-gray-600 p-2 rounded-lg text-indigo-500 shadow-sm">
                      ...
                      <div className="p-3 bg-gray-50 dark:bg-gray-700/50 rounded-xl border border-gray-100 dark:border-gray-600 flex items-center gap-3 flex-wrap">
                        <div className="bg-white dark:bg-gray-600 p-2 rounded-lg text-slate-500 shadow-sm">
                          <svg
                        ...
                        <div>
                          ...
                          <p className="text-xs font-bold text-gray-800 dark:text-gray-200 break-all">
                            {staff.email || "N/A"}
                          </p>
                        </div>
                      </div>
                      <div className="p-3 bg-gray-50 dark:bg-gray-700/50 rounded-xl border border-gray-100 dark:border-gray-600 flex items-center gap-3">
                        <div className="bg-white dark:bg-gray-600 p-2 rounded-lg text-pink-500 shadow-sm">
                      ...
                      <div className="p-3 bg-gray-50 dark:bg-gray-700/50 rounded-xl border border-gray-100 dark:border-gray-600 flex items-center gap-3">
                        <div className="bg-white dark:bg-gray-600 p-2 rounded-lg text-blue-500 shadow-sm">
                      ...
                      <div className="p-3 bg-gray-50 dark:bg-gray-700/50 rounded-xl border border-gray-100 dark:border-gray-600 flex items-center gap-3">
                        <div className="bg-white dark:bg-gray-600 p-2 rounded-lg text-orange-500 shadow-sm">
                        ...
                        <div>
                          ...
                          <p
                            className="text-xs font-bold text-gray-800 dark:text-gray-200 truncate max-w-[100px]"
                            title={staff.lastDevice}
                          >
                            {staff.lastDevice || "Unknown"}
                          </p>
                        </div>
                      </div>
                    </div>

                    {/* Profile Notes Section in Expanded Card */}
                    <div
                      className="mt-2 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-4 cursor-default"
                    ...
                    >
                      ...
                      <div className="bg-white dark:bg-gray-900 rounded-lg p-3 border border-gray-200 dark:border-gray-700 mb-4 shadow-sm">
                        <textarea
                          className="w-full bg-transparent text-xs text-gray-800 dark:text-gray-200 outline-none resize-none placeholder:text-gray-400 mb-2"
                        ...
                        {newNoteImage && (
                          <div className="relative inline-block mb-2">
                            <img
                              src={newNoteImage}
                              alt="Attachment"
                              className="h-16 rounded-md object-cover border border-gray-200 dark:border-gray-700"
                            />
                            <button
                        ...
                        <div className="flex items-center justify-between border-t border-gray-100 dark:border-gray-800 pt-2">
                          <button
                            ...
                            className="text-gray-500 hover:text-indigo-600 dark:text-gray-400 dark:hover:text-indigo-400 transition-colors p-1.5 rounded-md hover:bg-gray-100 dark:hover:bg-gray-800 flex items-center gap-1.5 text-[10px] font-bold"
                          >
                          ...
                          <button
                            ...
                            disabled={!newNoteText.trim() && !newNoteImage}
                            className="bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed text-white px-3 py-1.5 rounded-md text-[10px] font-bold transition-all shadow-md"
                          >
                            Save
                          </button>
                        </div>
                      ...
                      <div className="space-y-3 max-h-48 overflow-y-auto custom-scrollbar pr-1">
                        {staff.notes?.map((note, index) => (
                          <div
                            key={`${note.id}-${index}`}
                            className="bg-white dark:bg-gray-900 rounded-lg p-3 border border-gray-100 dark:border-gray-800 shadow-sm"
                          >
                            ...
                            {note.imageUrl && (
                              <img
                                src={note.imageUrl}
                                alt="Attachment"
                                className="mt-2 max-w-[120px] h-auto rounded-lg border border-gray-200 dark:border-gray-700 cursor-pointer hover:opacity-80 transition-opacity"
                                onClick={() =>
                                  setViewingNoteImage(note.imageUrl as string)
                                }
                              />
                            )}
                          </div>
                    ...
                    <div className="mt-auto pt-4 border-t border-gray-100 dark:border-gray-700">
                      <div className="flex gap-2 justify-between">
                        <button
                          ...
                          className="flex-1 bg-gray-50 dark:bg-gray-700 hover:bg-indigo-600 hover:text-white text-gray-600 dark:text-gray-300 py-3 rounded-xl flex flex-col items-center justify-center gap-1 transition-all group"
                        >
                        ...
                        <button
                          ...
                          className="flex-1 bg-gray-50 dark:bg-gray-700 hover:bg-gray-800 hover:text-white text-gray-600 dark:text-gray-300 py-3 rounded-xl flex flex-col items-center justify-center gap-1 transition-all group"
                        >
                        ...
                        {canManageMoney && (
                          <>
                            <button
                              onClick={(e) => {
                                e.stopPropagation();
                                openAdvanceModal(staff.id);
                              }}
                              className="flex-1 bg-blue-50 dark:bg-blue-900/20 hover:bg-blue-600 hover:text-white text-blue-600 dark:text-blue-400 py-3 rounded-xl flex flex-col items-center justify-center gap-1 transition-all group"
                            >
                            ...
                            <button
                              onClick={(e) => {
                                e.stopPropagation();
                                openRepayModal(staff.id);
                              }}
                              className="flex-1 bg-green-50 dark:bg-green-900/20 hover:bg-green-600 hover:text-white text-green-600 dark:text-green-400 py-3 rounded-xl flex flex-col items-center justify-center gap-1 transition-all group"
                            >
                            ...
                            <button
                              onClick={(e) => {
                                e.stopPropagation();
                                openGiftModal(staff.id);
                              }}
                              className="flex-1 bg-yellow-50 dark:bg-yellow-900/20 hover:bg-yellow-500 hover:text-white text-yellow-600 dark:text-yellow-400 py-3 rounded-xl flex flex-col items-center justify-center gap-1 transition-all group"
                            >
                        ...
                        {!isStaff && role === UserRole.ADMIN && (
                          <>
                            ...
                            <button
                              onClick={(e) => {
                                e.stopPropagation();
                                handleDeleteRequest(staff.id);
                              }}
                              className="flex-1 bg-red-50 dark:bg-red-900/20 hover:bg-red-600 hover:text-white text-red-500 dark:text-red-400 py-3 rounded-xl flex flex-col items-center justify-center gap-1 transition-all group"
                            >
                ...
                >
                  ...
                  <div className="flex flex-col items-center text-center h-full justify-center gap-3 relative z-10">
                    <div className="relative">
                      {staff.photo ? (
                        <img
                          src={staff.photo}
                          alt={safeName}
                          className={`w-16 h-16 rounded-full object-cover border-2 shadow-md ${isActive ? "border-white ring-2 ring-green-100 dark:ring-green-900" : "border-gray-200 grayscale"}`}
                        />
                      ) : (
                        <div
                          className={`w-16 h-16 rounded-full flex items-center justify-center text-xl font-bold text-white shadow-md ${isActive ? "bg-gradient-to-br from-indigo-400 to-violet-500" : "bg-gray-400"}`}
                        >
                          {safeName.charAt(0)}
                        </div>
                      )}
                      <div
                        className={`absolute bottom-0 right-0 w-4 h-4 rounded-full border-2 border-white dark:border-gray-800 shadow-sm ${isActive ? "bg-green-500" : "bg-red-500"}`}
                      ></div>
                    </div>
                    <div>
                      <h4 className="text-sm font-black text-gray-800 dark:text-white truncate max-w-[140px] leading-tight">
                        {safeName}
                      </h4>
                      <p className="text-[10px] font-bold text-indigo-600 dark:text-indigo-400 mt-0.5 truncate max-w-[140px]">
                        {staff.designation}
                      </p>
                    ...
                    {staff.role !== UserRole.STAFF && (
                      <span
                        className={`text-[8px] font-black uppercase px-2 py-0.5 rounded-full ${staff.role === UserRole.MD ? "bg-purple-100 text-purple-700" : "bg-blue-100 text-blue-700"}`}
                      >
                        {staff.role}
                      </span>
                    )}
          ...
          <div className="bg-white w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-indigo-600 text-white shrink-0">
              <h3 className="font-bold text-xl">
                {editingStaff ? "প্রোফাইল আপডেট" : "নতুন ইউজার যুক্ত করুন"}
              </h3>
              <button
                onClick={closeModal}
            ...
            >
              <div className="p-6 space-y-6 overflow-y-auto custom-scrollbar flex-1">
                <div className="flex flex-col items-center">
                  <div
                    ...
                    className="w-24 h-24 rounded-full border-4 border-indigo-50 flex items-center justify-center cursor-pointer hover:border-indigo-200 overflow-hidden relative group shadow-lg"
                  >
                    <input
                      ref={fileInputRef}
                      type="file"
                ...
                <div className="space-y-6">
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <input
                        required
                        type="text"
                        className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none transition-all text-sm font-semibold placeholder:text-gray-400"
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <input
                          type="tel"
                          className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none text-sm font-semibold"
                      ...
                      <div>
                        <label className="block text-xs font-bold text-gray-700 mb-1">
                          ইমেইল{" "}
                          <span className="font-normal text-gray-400 font-sans tracking-normal ml-1">
                            (Optional)
                          </span>
                        </label>
                        <input
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="block text-xs font-bold text-gray-700 mb-1">
                          স্টাফ আইডি <span className="text-red-500">*</span>
                        </label>
                        <input
                          required
                          type="text"
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <div className="relative">
                          ...
                          <input
                            type="number"
                            className="w-full pl-8 pr-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none text-sm font-semibold"
                            disabled={isStaff}
                      ...
                      <div>
                        ...
                        <div className="relative">
                          ...
                          >
                            ...
                            <option value={UserRole.MD}>
                              MD (Super Admin)
                            </option>
                            <option value={UserRole.KIOSK}>
                              Kiosk / Factory Device
                            </option>
                          </select>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="bg-indigo-50/50 p-4 rounded-xl border border-indigo-100 space-y-4">
                      <div>
                        ...
                        <div className="relative">
                          ...
                          >
                            ...
                            <option value="FIELD">
                              ফিল্ড ওয়ার্ক (কোনো লোকেশন নেই)
                            </option>
                            <option value="CUSTOM">
                              কাস্টম লোকেশন (ম্যাপ সেটআপ)
                            </option>
                          </select>
                          <ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 w-3 h-3 text-gray-400 pointer-events-none" />
                        </div>
                        ...
                        <div className="space-y-4 animate-in fade-in slide-in-from-top-2 pt-2">
                          ...
                          <div className="bg-white p-3 rounded-xl border border-indigo-100 shadow-sm">
                            <div className="flex justify-between items-center mb-2 border-b border-gray-50 pb-2">
                              ...
                              <button
                                ...
                                className="text-[10px] font-bold text-white bg-indigo-500 px-2.5 py-1 rounded-lg flex items-center gap-1 hover:bg-indigo-600 transition-colors shadow-sm shadow-indigo-200"
                              >
                            ...
                            <div className="space-y-2">
                              <input
                                type="text"
                                placeholder="Location Name (e.g. Uttara Office)"
                                className="w-full px-3 py-2 border border-gray-200 rounded-lg text-xs font-medium focus:border-indigo-300 outline-none"
                                value={formData.customLocName}
                                onChange={(e) =>
                                  setFormData({
                                    ...formData,
                                    customLocName: e.target.value,
                                  })
                                }
                              />
                              <div className="grid grid-cols-2 gap-2">
                                <input
                                  ...
                                  onChange={(e) =>
                                    setFormData({
                                      ...formData,
                                      customLat: Number(e.target.value),
                                    })
                                  }
                                />
                                <input
                                  ...
                                  onChange={(e) =>
                                    setFormData({
                                      ...formData,
                                      customLng: Number(e.target.value),
                                    })
                                  }
                                />
                              </div>
                              <div className="flex items-center gap-2 bg-gray-50 p-2 rounded-lg">
                                ...
                                <input
                                  type="number"
                                  className="w-full px-2 py-1 border border-gray-200 rounded text-xs font-bold text-center"
                                  value={formData.customRadius}
                                  onChange={(e) =>
                                    setFormData({
                                      ...formData,
                                      customRadius: Number(e.target.value),
                                    })
                                  }
                                />
                              </div>
                          ...
                          <div className="flex items-center gap-2 pl-1">
                            <div className="relative flex items-center">
                              <input
                                type="checkbox"
                                id="hasSecondLoc"
                                className="peer w-4 h-4 text-indigo-600 rounded border-gray-300 focus:ring-indigo-500"
                                checked={formData.hasSecondLoc}
                                onChange={(e) =>
                                  setFormData({
                                    ...formData,
                                    hasSecondLoc: e.target.checked,
                                  })
                                }
                              />
                              <label
                                htmlFor="hasSecondLoc"
                                className="ml-2 text-xs font-bold text-gray-700 cursor-pointer select-none"
                              >
                                ২য় লোকেশন যুক্ত করুন (Optional)
                              </label>
                            </div>
                          </div>
                          ...
                          {formData.hasSecondLoc && (
                            <div className="bg-white p-3 rounded-xl border border-purple-100 shadow-sm animate-in fade-in">
                              <div className="flex justify-between items-center mb-2 border-b border-gray-50 pb-2">
                                <p className="text-[10px] font-black text-purple-600 uppercase tracking-wider flex items-center gap-1">
                                  <MapPin className="w-3 h-3" /> Secondary
                                  Location
                                </p>
                                <button
                                  type="button"
                                  onClick={() => getCurrentLocation(2)}
                                  className="text-[10px] font-bold text-white bg-purple-500 px-2.5 py-1 rounded-lg flex items-center gap-1 hover:bg-purple-600 transition-colors shadow-sm shadow-purple-200"
                                >
                                  <LocateFixed className="w-3 h-3" /> Get
                                  Current
                                </button>
                              </div>
                              <div className="space-y-2">
                                <input
                                  type="text"
                                  placeholder="Location Name (e.g. Mirpur Branch)"
                                  className="w-full px-3 py-2 border border-gray-200 rounded-lg text-xs font-medium focus:border-purple-300 outline-none"
                                  value={formData.customLocName2}
                                  onChange={(e) =>
                                    setFormData({
                                      ...formData,
                                      customLocName2: e.target.value,
                                    })
                                  }
                                />
                                <div className="grid grid-cols-2 gap-2">
                                  <input
                                    ...
                                    placeholder="Lat"
                                    className="w-full px-3 py-2 border border-gray-200 rounded-lg text-xs font-mono"
                                    value={formData.customLat2}
                                    onChange={(e) =>
                                      setFormData({
                                        ...formData,
                                        customLat2: Number(e.target.value),
                                      })
                                    }
                                  />
                                  <input
                                    ...
                                    placeholder="Lng"
                                    className="w-full px-3 py-2 border border-gray-200 rounded-lg text-xs font-mono"
                                    value={formData.customLng2}
                                    onChange={(e) =>
                                      setFormData({
                                        ...formData,
                                        customLng2: Number(e.target.value),
                                      })
                                    }
                                  />
                                </div>
                                <div className="flex items-center gap-2 bg-gray-50 p-2 rounded-lg">
                                  ...
                                  <input
                                    type="number"
                                    className="w-full px-2 py-1 border border-gray-200 rounded text-xs font-bold text-center"
                                    value={formData.customRadius2}
                                    onChange={(e) =>
                                      setFormData({
                                        ...formData,
                                        customRadius2: Number(e.target.value),
                                      })
                                    }
                                  />
                                </div>
                      ...
                      <div className="flex items-center gap-3 pt-2 border-t border-indigo-100/50">
                        <div
                          className={`w-8 h-4 rounded-full relative transition-colors duration-300 cursor-pointer ${formData.requiresCheckOutLocation ? "bg-indigo-500" : "bg-gray-300"}`}
                          onClick={() =>
                            !isStaff &&
                            setFormData({
                              ...formData,
                              requiresCheckOutLocation:
                                !formData.requiresCheckOutLocation,
                            })
                          }
                        >
                          <div
                            className={`absolute top-0.5 w-3 h-3 bg-white rounded-full shadow-sm transition-transform duration-300 ${formData.requiresCheckOutLocation ? "left-4.5" : "left-0.5"}`}
                          ></div>
                        </div>
                        <label
                        ...
                        >
                          চেক-আউট এর সময় লোকেশন চেক বাধ্যতামূলক?
                        </label>
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="bg-gray-50 p-4 rounded-xl border border-gray-100">
                      ...
                      <div className="relative">
                        <input
                          ...
                          className="w-full pl-4 pr-10 py-2.5 bg-white border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none text-sm font-mono"
                          ...
                          placeholder={
                            editingStaff
                              ? "পরিবর্তন করতে চাইলে লিখুন"
                              : "Default: নাম@ (উদা: রহিম@)"
                          }
                        />
                        <button
                          type="button"
                      ...
                      <p className="text-[10px] text-gray-400 mt-1.5 ml-1">
                        * ডিফল্ট পাসওয়ার্ড সাধারণত নামের সাথে @ যুক্ত করে হয়।
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <div className="p-6 border-t border-gray-100 bg-gray-50 flex gap-3 shrink-0">
                ...
                <button
                  type="submit"
                  className="flex-[2] bg-indigo-600 text-white py-3 rounded-xl font-bold hover:bg-indigo-700 shadow-lg shadow-indigo-100 transition-all active:scale-95 flex items-center justify-center gap-2"
                >
                  {editingStaff ? (
                    <>
                      <Edit3 className="w-4 h-4" /> আপডেট করুন
                    </>
                  ) : (
                    <>
                      <UserPlus className="w-4 h-4" /> সেভ করুন
                    </>
                  )}
                </button>
              </div>
          ...
          <div className="bg-white dark:bg-gray-800 w-full max-w-sm rounded-2xl shadow-2xl overflow-hidden">
            <div className="p-6 text-center">
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
                আপনি এই স্টাফ মেম্বারকে ডিলিট করতে চাচ্ছেন। এটি রিসাইকেল বিনে
                জমা হবে।
              </p>
              ...
              <div className="mb-6 relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-3 bg-gray-50 dark:bg-gray-700/50 border border-gray-200 dark:border-gray-600 rounded-xl outline-none focus:ring-2 focus:ring-red-500 dark:text-gray-200"
      ...
      {statusConfirmData && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white dark:bg-gray-800 w-full max-w-sm rounded-2xl shadow-2xl overflow-hidden">
            <div className="p-6 text-center">
              <div
                className={`w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4 ${statusConfirmData.newStatus === "DEACTIVATED" ? "bg-orange-100 dark:bg-orange-900/30 text-orange-600 dark:text-orange-400" : "bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400"}`}
              >
            ...
            <form onSubmit={handleGiveAdvance} className="space-y-4">
              <div>
                ...
                <input
                  required
                  type="number"
                  className="w-full p-2 border rounded-lg dark:bg-gray-900 dark:border-gray-700 dark:text-white"
            ...
            <form onSubmit={handleRepay} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setRepayFormData({ ...repayFormData, note: e.target.value })
                  }
                />
              </div>
              <div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setRepayFormData({ ...repayFormData, date: e.target.value })
                  }
                />
              </div>
              <div className="flex gap-2 pt-2">
          ...
          <div className="bg-white dark:bg-gray-800 w-full max-w-sm rounded-2xl shadow-2xl overflow-hidden p-6 text-center">
            ...
            <form onSubmit={handleGivePoints} className="space-y-4">
              <input
                required
                type="number"
                className="w-full p-3 border rounded-xl text-center text-2xl font-bold dark:bg-gray-900 dark:border-gray-700 dark:text-white"
              ...
              <div className="flex gap-2">
                <button
                  type="button"
                  onClick={() => setIsGiftPointModalOpen(false)}
                  className="flex-1 py-2 border rounded-lg text-gray-500 dark:text-gray-400"
                >
                  বাতিল
                </button>
                <button
                  ...
                  className="flex-1 py-2 bg-yellow-500 text-white rounded-lg font-bold"
                >
                  পয়েন্ট দিন
                </button>
              </div>
              <button
                type="button"
                onClick={() => {
                  setPointMode("PENALTY");
                  handleGivePoints({ preventDefault: () => {} } as any);
                }}
                className="text-xs text-red-500 hover:underline"
              >
                পয়েন্ট কাটুন (Penalty)
              </button>
            </form>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white dark:bg-gray-800 w-full max-w-sm rounded-2xl shadow-2xl p-6 text-center">
            ...
            <p className="text-sm text-gray-500 mb-4">
              আপনি কি নিশ্চিত যে {giftPointData.points} পয়েন্ট কেটে নিতে চান?
            </p>
            <div className="flex gap-3">
              <button
      ...
      {historyStaff &&
        createPortal(
          ...
          >
            <div
              className="bg-white dark:bg-gray-800 w-full max-w-xl rounded-3xl shadow-2xl overflow-hidden max-h-[85vh] flex flex-col border border-gray-200 dark:border-gray-700"
              onClick={(e) => e.stopPropagation()}
            ...
            >
                ...
                <div className="relative z-10 flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs font-bold text-indigo-600 dark:text-indigo-400 uppercase tracking-wider">
                      {historyStaff.designation}
                    </p>
                  </div>
                </div>
                <button
                  onClick={() => setHistoryStaff(null)}
                  className="relative z-10 p-2 bg-white/50 dark:bg-gray-800/50 hover:bg-white dark:hover:bg-gray-700 rounded-full transition-all shadow-sm border border-gray-200 dark:border-gray-600 active:scale-95"
                >
              ...
              <div className="flex-1 overflow-y-auto p-5 custom-scrollbar bg-gray-50/50 dark:bg-gray-800/50">
                ...
                {(() => {
                  const { balance, totalSalaryAdv, totalRegularAdv, approved } =
                    getStaffFinancials(historyStaff.id);
                  return (
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 mb-3">
                  ...
                  <div className="relative w-full md:w-auto">
                    ...
                    <input
                      ...
                      className="w-full md:w-48 pl-8 pr-3 py-1.5 text-xs bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg outline-none focus:ring-2 focus:ring-indigo-500 dark:text-white"
                ...
                {(() => {
                  const staffExpenses = expenses
                    .filter(
                      (e) => e.staffId === historyStaff.id && !e.isDeleted,
                    )
                    .map((e) => ({ ...e, type: "EXPENSE", date: e.createdAt }));
                  const staffAdvances = advances
                    .filter(
                      (a) => a.staffId === historyStaff.id && !a.isDeleted,
                    )
                    .map((a) => ({ ...a, type: "ADVANCE", date: a.date }));

                  let historyItems = [...staffExpenses, ...staffAdvances].sort(
                    (a, b) =>
                  ...
                  if (historySearchTerm) {
                    ...
                    historyItems = historyItems.filter(
                      (item: any) =>
                        (item.reason || "").toLowerCase().includes(lowerTerm) ||
                  ...
                  return (
                    <div className="space-y-3">
                      {historyItems.map((item: any, index: number) => (
                        <div
                          key={`${item.id}-${index}`}
                          className="bg-white dark:bg-gray-800 p-4 rounded-2xl border border-gray-100 dark:border-gray-700 flex justify-between items-center hover:shadow-md transition-all duration-300 group"
                        >
                          <div>
                            <div className="flex items-center gap-2 mb-1.5">
                              <span
                                className={`text-[9px] font-black px-2 py-0.5 rounded uppercase tracking-wider border ${item.type === "EXPENSE" ? "bg-orange-50 text-orange-600 border-orange-200" : item.type === "SALARY" ? "bg-purple-50 text-purple-600 border-purple-200" : "bg-blue-50 text-blue-600 border-blue-200"}`}
                              >
                                {item.type === "EXPENSE"
                                  ? "BILL"
                                  : item.type === "SALARY"
                            ...
                            <p className="text-sm font-bold text-gray-800 dark:text-gray-200 line-clamp-1 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">
                              {item.reason || item.note || "No description"}
                            </p>
                          </div>
                          <div className="text-right">
                            <p
                              className={`text-base font-black tracking-tight ${item.type === "EXPENSE" ? "text-red-500" : "text-emerald-600"}`}
                            >
                              ৳ {item.amount.toLocaleString("en-US")}
                            </p>
                            {item.type === "EXPENSE" && (
                              <span
                                className={`text-[9px] font-black uppercase tracking-wider ${item.status === "APPROVED" ? "text-emerald-500" : item.status === "REJECTED" ? "text-red-500" : "text-orange-500"}`}
                              >
                                {item.status}
                              </span>
                            )}
          ...
          >
            <div className="relative max-w-4xl w-full h-full flex justify-center items-center">
              <button
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  setViewingNoteImage(null);
                }}
              >
                <X className="w-6 h-6" />
              </button>
              <img
                src={viewingNoteImage}
                alt="Full View"
        ...
        createPortal(
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white dark:bg-slate-800 w-full max-w-md rounded-2xl shadow-2xl overflow-hidden flex flex-col">
              <div className="p-6 border-b border-gray-100 dark:border-gray-700 flex items-center gap-4 bg-purple-50 dark:bg-purple-900/10">
                ...
                <div>
                  <h3 className="text-xl font-bold text-gray-800 dark:text-white leading-none mb-1">
                    প্রোফাইল মার্জ
                  </h3>
                  <p className="text-xs text-gray-500 dark:text-gray-400">
                    দুটি স্টাফ প্রোফাইলের সকল ডাটা একত্র করুন
                  </p>
                </div>
              </div>
              <form onSubmit={handleMergeProfiles} className="p-6 space-y-6">
                <div>
                  <label className="block text-xs font-bold text-red-500 uppercase tracking-widest mb-2">
                    সোর্স প্রোফাইল (যাকে ডিলিট করা হবে)
                  </label>
                  <select
                    required
                    className="w-full p-3 bg-red-50 dark:bg-red-900/10 border border-red-200 dark:border-red-800 rounded-xl outline-none focus:ring-2 focus:ring-red-500 dark:text-gray-200 text-sm font-semibold"
                    value={sourceStaffId}
                ...
                <div className="flex justify-center -my-3 relative z-10 text-gray-400">
                  <div className="bg-white dark:bg-slate-800 p-2 rounded-full border border-gray-200 dark:border-gray-700 shadow-sm">
                    <ArrowRight className="w-5 h-5 rotate-90" />
                  </div>
                </div>
                <div>
                  <label className="block text-xs font-bold text-emerald-600 uppercase tracking-widest mb-2">
                    টার্গেট প্রোফাইল (যেখানে ডাটা যাবে)
                  </label>
                  <select
                    required
                    className="w-full p-3 bg-emerald-50 dark:bg-emerald-900/10 border border-emerald-200 dark:border-emerald-800 rounded-xl outline-none focus:ring-2 focus:ring-emerald-500 dark:text-gray-200 text-sm font-semibold"
                    value={targetStaffId}
                ...
                <div className="bg-orange-50 dark:bg-orange-900/20 p-4 border border-orange-100 dark:border-orange-800 rounded-xl flex gap-3 text-sm text-orange-800 dark:text-orange-300">
                  <AlertTriangle className="w-5 h-5 shrink-0 text-orange-500" />
                  <p>
                    সোর্স প্রোফাইলের সকল অ্যাডভান্স, এক্সপেন্স এবং মুভমেন্ট
                    টার্গেট প্রোফাইলে চলে যাবে। এরপর সোর্স প্রোফাইলটি
                    স্বয়ংক্রিয়ভাবে ডিলিট হয়ে যাবে।
                  </p>
                </div>
                ...
                <div>
                  ...
                  <input
                    type="password"
                    required
                    className="w-full p-3 bg-gray-50 dark:bg-slate-700 border border-gray-200 dark:border-gray-600 rounded-xl outline-none focus:ring-2 focus:ring-purple-500 dark:text-white"
                    value={mergePassword}
                ...
                <div className="flex gap-3 pt-2">
                  <button
                    ...
                    className="flex-1 py-3 border border-gray-200 dark:border-gray-700 rounded-xl font-bold text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-slate-700"
                  >
                    বাতিল
                  </button>
                  <button
                    type="submit"
                    className="flex-1 py-3 bg-purple-600 hover:bg-purple-700 text-white rounded-xl font-bold shadow-lg shadow-purple-200 dark:shadow-none transition-colors active:scale-95"
                  >
                    মার্জ করুন
                  </button>
                </div>
